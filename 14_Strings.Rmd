...
`/n` for a new line aka. Enter `/t` for a new Tab aka. Tab `/u` or `/U` for Unicode. Check `?Quotes` for doc.
...
# use vector to show both in one function
...
Shows squigly braces, just like for `/t` to show that the empty space is a string/

    3. google this special character It's called a *No-Break Space*. Space and now line brake allowed.

```{r}
x <- "This/u00a0is/u00a0tricky"
```
...
has one step more, where the column names need to be defined.
Also the raw data needs the same number of "pieces".
```{r}
...
Trends in first and last Letters
```{r}
...
```
I would like to have a ranking for each letter and then check the average top three or so.
```{r}
# try first letter only first
babynames |> 
  mutate(fl = str_sub(name, 1, 1)) |> 
  group_by(year, fl) |> 
  count(wt = n) |>
...
babynames |> 
  ...
  count(year, fl, wt = n) |>
  #filter(year >= 1980) |> 
  slice_max(n = 3, by = year, order_by = n) |> 
  ggplot(aes(year, n)) +
...
```{r}
# test some stuff
# add rank for some value
df <- tibble(x = c(1:15),
             y = C(2:16))
df |> mutate(rank = min_rank(desc(x)))

# okay add columns with weighted counts for first and last letter
babynames |> 
  mutate(fl = str_sub(name, 1, 1),
         ll = str_sub(name, -1, -1)
  ) |> 
  add_count(year, fl, wt = n, name = "fln") |> 
  add_count(year, ll, wt = n, name = "lln") |> 
  # add rank for first and last letter
  mutate(.by = year,
         frank = min_rank(desc(fln)),
         lrank = min_rank(desc(lln)),
         ) |> 
  # check the ranking
  summarise(mean(lrank))
# looks off there are only 26 letters, so how 8k ranks?
# test again with one letter
babynames |> 
  mutate(fl = str_sub(name, 1,1)) |> 
  add_count(year, fl, wt = n, name = "fln") |> 
  group_by(year, fl) |> 
  mutate(min_rank(fln))
## this aint right, but i'm two hours in. Let's leave it with the sinple ggplot
```

## 14.6.3 Non-English text
See how strings are encoded for computing
```{r}
charToRaw("Bernd Erich")
```
UTF-8 is the encoding standard, which is supported with a lot of languages and also emojis.
If text was encoded in an older system, find and call the original system and also locale to convert from.
```{r}

x2 <- "text\n\x82\xb1\x82\xf1\x82\xc9\x82\xbf\x82\xcd"

x1 <- "text\nEl Ni\xf1o was particularly bad this year"
read_csv(x1, delim = \\)$text
